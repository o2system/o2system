...
(function($){
    $(document).ready(function() {
        $('.nav > li > i').on('click touchend', function(e) {
            e.preventDefault();
            var $self = $(this);
            var $section = $self.parent();
            if(!$section.hasClass('current') && $section.siblings().hasClass('current')) {
                $section.addClass('current').find('ul').animate({'height':'show'}, 500);
                $section.siblings().removeClass('current').find('ul').animate({'height':'hide'}, 500);
            } else if ($section.hasClass('current') && !$section.siblings().hasClass('current')) {
                $section.removeClass('current').find('ul').animate({'height':'hide'}, 500);
            } else if (!$section.hasClass('current') && !$section.siblings().hasClass('current')) {
                $section.addClass('current').find('ul').animate({'height':'show'}, 500);
            }
        });

        $('.nav > li > ul > i').on('click touchend', function(e) {
            e.preventDefault();
            var $self = $(this);
            var $section = $self.parent();
            if(!$section.hasClass('current') && $section.siblings().hasClass('current')) {
                ...
                $section.removeClass('current').find('ul').animate({'height':'hide'}, 500);
            } else if (!$section.hasClass('active') && !$section.siblings().hasClass('current')) {
                $section.addClass('current').find('ul').animate({'height':'show'}, 500);
            }
        });
        /*Mobile menu*/
    ...
    $('.sidebar-menu-container').slimScroll({
        position: 'right'
        , size: "5px"
        , height: '100%'
        , color: '#dcdcdc'
